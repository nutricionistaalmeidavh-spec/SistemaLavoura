...
export async function createStandaloneHost({dataDir,backupDir=join(dataDir,'backups')}={}){
  ...
  const call=(name,args)=>{
    if(!current)throw new Error('Persistence is quiesced.');
    const fn=current[name];
    if(typeof fn!=='function')throw new TypeError(`Persistence method ${name} is unavailable.`);
    return fn.apply(current,args);
  };
  const persistence=Object.freeze({
    productId:PRODUCT_ID,
    applySeed:(...a)=>call('applySeed',a),
    ...
    runInTransaction:(work)=>call('runInTransaction',[work])
  });
  async function quiesced(work){
    const old=current;
    current=null;
    await old?.close?.();
    try{return await work();}finally{current=await open();}
  }
  async function captureAuditRecords(){
    const collections=(await persistence.listCollections({includeSystem:true})).filter(isAuditCollection);
    const out=[];
    for(const collection of collections){
      for(const record of await persistence.listRecords(collection,{includeDeleted:true}))out.push(record);
    }
    return out;
  }
  async function mergeAuditRecords(records){
    for(const record of records){
      const existing=await persistence.getRecord(record.collection,record.id,{includeDeleted:true});
      if(!existing)await persistence.putRecord(record.collection,record.id,record.payload,{expectedVersion:0});
    }
  }
  const recovery=Object.freeze({
    async createBackup({id=`backup-${Date.now()}`}={}){
      const backupId=safeId(id),path=join(backupDir,`${backupId}.sqlite`);
      await quiesced(()=>copyFile(dbPath,path));
      const info=await stat(path);
      return {id:backupId,createdAt:info.mtime.toISOString(),path};
    },
    async listBackups(){
      const names=(await readdir(backupDir)).filter(name=>name.endsWith('.sqlite')).sort().reverse();
      return Promise.all(names.map(async name=>{const path=join(backupDir,name),info=await stat(path);return{id:name.slice(0,-7),createdAt:info.mtime.toISOString(),path};}));
    },
    async restoreBackup(id){
      const backupId=safeId(id),source=join(backupDir,`${backupId}.sqlite`),safety=join(backupDir,`safety-${Date.now()}.sqlite`);
      const auditRecords=await captureAuditRecords();
      await quiesced(async()=>{await copyFile(dbPath,safety);await copyFile(source,dbPath);});
      await mergeAuditRecords(auditRecords);
      return{restored:true,backupId,safetyBackupId:safety.split(/[\\/]/).at(-1).replace(/\.sqlite$/,'')};
    }
  });
