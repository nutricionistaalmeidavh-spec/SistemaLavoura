...
import {createAgroLavouraPresentation} from '../src/presentation-p5.js';
...
export async function createStandaloneHost({dataDir,backupDir=join(dataDir,'backups')}={}){
  ...
  async function captureAuditRecords(){
    ...
    const out=[];
    for(const collection of collections){for(const record of await persistence.listRecords(collection,{includeDeleted:true}))out.push(record);}
    return out;
  }
  async function mergeAuditRecords(records){
    for(const record of records){const existing=await persistence.getRecord(record.collection,record.id,{includeDeleted:true});if(!existing)await persistence.putRecord(record.collection,record.id,record.payload,{expectedVersion:0});}
  }
  const recovery=Object.freeze({
    ...
    async restoreBackup(id){const backupId=safeId(id),source=join(backupDir,`${backupId}.sqlite`),safety=join(backupDir,`safety-${Date.now()}.sqlite`),auditRecords=await captureAuditRecords();await quiesced(async()=>{await copyFile(dbPath,safety);await copyFile(source,dbPath);});await mergeAuditRecords(auditRecords);return{restored:true,backupId,safetyBackupId:safety.split(/[\\/]/).at(-1).replace(/\.sqlite$/,'')};}
  });
