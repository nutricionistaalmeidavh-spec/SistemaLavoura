...
export function createMapPackageManager({dataDir,fetchImpl=globalThis.fetch,execFileImpl=execFileAsync,platform=process.platform,arch=process.arch,now=()=>new Date()}={}){
  ...
  async function loadCachedCatalog(){if(catalogCache)return catalogCache;if(await exists(catalogPath)){try{catalogCache=JSON.parse(await readFile(catalogPath,'utf8'));}catch{catalogCache=null;}}return catalogCache;}
  async function refreshCatalog(url=DEFAULT_MANIFEST_URL){await init();const response=await fetchImpl(url,{redirect:'follow'});if(!response.ok)throw new Error(`Map catalog unavailable (${response.status}). Publish a map release or configure another catalog.`);const value=await response.json();if(value?.schemaVersion!==1||!Array.isArray(value?.maps))throw new Error('Invalid map catalog response.');catalogCache=value;await atomicJson(catalogPath,value);return value;}
  async function resolveRecentSource(days=7){
    ...
    throw new Error(`No Protomaps daily build was reachable in the last ${days} days.`);
  }
  async function ensureCli(){
    await init();if(await exists(cliPath))return cliPath;
    if(platform!=='win32'||arch!=='x64')throw new Error('Automatic farm map extraction is currently available on Windows x64 desktop.');
    await mkdir(cliDir,{recursive:true});
    const response=await fetchImpl(WINDOWS_X64_ASSET.url,{redirect:'follow'});if(!response.ok)throw new Error(`Failed to download PMTiles CLI (${response.status}).`);
    const buffer=Buffer.from(await response.arrayBuffer());const digest=sha256Buffer(buffer);if(digest!==WINDOWS_X64_ASSET.sha256)throw new Error('PMTiles CLI checksum mismatch.');
    ...
    const candidates=[join(expandDir,'pmtiles.exe'),join(expandDir,'pmtiles')];let source=null;for(const candidate of candidates)if(await exists(candidate)){source=candidate;break;}if(!source)throw new Error('PMTiles CLI executable was not found in the verified archive.');
  ...
  async function installFarmMap(input={}){
    ...
    try{
      ...
      const finalPath=join(packages,plan.outputAsset),tempPath=`${finalPath}.part-${process.pid}-${Date.now()}`,backupPath=`${finalPath}.bak`;
