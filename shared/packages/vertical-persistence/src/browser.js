...
export function createBrowserPersistence({storage=null,indexedDB=globalThis.indexedDB,productId,dbName=null}={}){
  ...
  return Object.freeze({
    productId:pid,
    storage:scoped,
    async putRecord(collection,id,payload,{expectedVersion}={}){ensureOpen();const c=text(collection,'Collection'),key=text(id,'Record id');const current=await this.getRecord(c,key,{includeDeleted:true});const version=current?.version??0;if(expectedVersion!==undefined&&expectedVersion!==version){const error=new Error('version conflict');error.code='VERSION_CONFLICT';throw error;}const next=Object.freeze({collection:c,id:key,payload:structuredClone(payload),version:version+1,deletedAt:null,updatedAt:nowIso()});return saveExact(next);},
    ...
    async softDeleteRecord(collection,id,{expectedVersion,at=nowIso()}={}){ensureOpen();const current=await this.getRecord(collection,id,{includeDeleted:true});if(!current)throw new Error('not found');if(expectedVersion!==undefined&&expectedVersion!==current.version){const error=new Error('version conflict');error.code='VERSION_CONFLICT';throw error;}const next=Object.freeze({...current,version:current.version+1,deletedAt:new Date(at).toISOString(),updatedAt:new Date(at).toISOString()});return saveExact(next);},
