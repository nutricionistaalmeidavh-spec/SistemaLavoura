...
function normalizeRing(geometry){
  const source=geometry?.coordinates;
  const candidate=Array.isArray(source?.[0]?.[0])?source[0]:source;
  ...
  for(const point of candidate){
    ...
    ring.push(Object.freeze([longitude,latitude]));
  }
...
function ringBounds(ring){
  ...
  return Object.freeze({minLongitude:Math.min(...longitudes),minLatitude:Math.min(...latitudes),maxLongitude:Math.max(...longitudes),maxLatitude:Math.max(...latitudes)});
}

function seasonForField(fieldId,seasons,now){
  const related=seasons.filter(item=>item.fieldId===fieldId||(item.fieldIds??[]).map(String).includes(String(fieldId)));
  if(!related.length)return null;
  const active=related.filter(item=>{
    const start=dateValue(item.plantingWindowStart??item.startsAt),end=dateValue(item.plantingWindowEnd??item.endsAt);
    if(start!=null&&now<start)return false;if(end!=null&&now>end)return false;return start!=null||end!=null;
  });
...
function coordinateFor(item,fieldById){
  const explicit=maybeCoordinate(item?.latitude,item?.longitude);if(explicit)return Object.freeze({...explicit,coordinateSource:'explicit'});
  const field=fieldById.get(String(item?.fieldId??''));if(!field?.centroid)return null;
  return Object.freeze({...field.centroid,coordinateSource:'field-centroid'});
}
...
export function createAgriculturalMapPoint(input={}){
  ...
  if(!MAP_POINT_KINDS.includes(kind))throw new TypeError(`Map point kind must be one of: ${MAP_POINT_KINDS.join(', ')}.`);
...
export function buildAgriculturalMapSnapshot(input={}){
  ...
  const geometryByField=new Map(geometries.map(item=>[String(item.fieldId??item.id),item]));
  const mappedFields=[];
  for(const field of fields){
    const ring=normalizeRing(geometryByField.get(String(field.id)));if(!ring)continue;
    const season=seasonForField(field.id,seasons,now),centroid=polygonCentroid(ring),bounds=ringBounds(ring);
    const fieldScouting=scouting.filter(item=>String(item.fieldId)===String(field.id));
    const fieldFiles=files.filter(item=>String(item.entityId??item.fieldId)===String(field.id)&&imageFile(item));
    const fieldRain=rainfall.filter(item=>String(item.fieldId)===String(field.id)&&(dateValue(item.measuredAt??item.recordedAt)??0)>=recentRainAfter);
    const fieldOperations=operations.filter(item=>String(item.fieldId)===String(field.id));
    const fieldApplications=applications.filter(item=>String(item.fieldId)===String(field.id));
    mappedFields.push(Object.freeze({fieldId:String(field.id),code:field.code??null,name:field.name??String(field.id),farmUnitId:field.farmUnitId??null,areaHa:Number(field.areaHa??0),geometry:Object.freeze({type:'Polygon',coordinates:ring}),centroid,bounds,season:seasonCard(season),color:season?.crop?colorFor(season.crop):'#718096',summary:Object.freeze({openScouting:fieldScouting.filter(item=>!['closed','resolved','dismissed'].includes(String(item.status??'open').toLowerCase())).length,photoCount:fieldFiles.length,recentRainMm:fieldRain.reduce((sum,item)=>sum+(Number(item.mm)||0),0),plannedOperations:fieldOperations.filter(item=>['planned','in-progress'].includes(String(item.status))).length,recentApplications:fieldApplications.filter(item=>(dateValue(item.appliedAt)??0)>=recentRainAfter).length})}));
  }
  const fieldById=new Map(mappedFields.map(item=>[String(item.fieldId),item]));
  const compact=values=>Object.freeze(values.filter(Boolean));
  const layers={
    applications:compact(applications.map(item=>marker(item,'application',fieldById,{name:item.target??'Aplicação',appliedAt:item.appliedAt??null,products:Object.freeze([...(item.products??[])])}))),
    scouting:compact(scouting.map(item=>marker(item,'scouting',fieldById,{name:item.name??item.kind??'Monitoramento',severity:Number(item.severity??0),status:item.status??'open',observedAt:item.observedAt??null}))),
    operations:compact(operations.map(item=>marker(item,'operation',fieldById,{name:item.typeName??item.typeId??'Operação',status:item.status??null,scheduledAt:item.scheduledAt??null,machineName:item.machineName??null}))),
    photos:compact(files.filter(imageFile).map(item=>marker({...item,fieldId:item.fieldId??item.entityId},'photo',fieldById,{name:item.name??'Foto',mimeType:item.mimeType??null}))),
    rainfall:compact(rainfall.map(item=>marker(item,'rainfall',fieldById,{name:'Chuva',mm:Number(item.mm??0),measuredAt:item.measuredAt??item.recordedAt??null}))),
    sensors:compact(mapPoints.filter(item=>item.kind==='sensor').map(item=>marker(item,'sensor',fieldById,{status:item.status??'active'}))),
    machines:compact(mapPoints.filter(item=>item.kind==='machine').map(item=>marker(item,'machine',fieldById,{status:item.status??'active'}))),
    storage:compact(mapPoints.filter(item=>item.kind==='storage').map(item=>marker(item,'storage',fieldById,{status:item.status??'active'}))),
    sampling:compact(mapPoints.filter(item=>item.kind==='sampling').map(item=>marker(item,'sampling',fieldById,{status:item.status??'active'})))
  };
  const allCoordinates=[...mappedFields.flatMap(field=>field.geometry.coordinates.map(([longitude,latitude])=>({longitude,latitude}))),...Object.values(layers).flatMap(items=>items.map(({longitude,latitude})=>({longitude,latitude})))];
