...
function validateCacheEntry(entity){
  if(!entity?.scene)throw new TypeError('Satellite cache entry requires a scene.');
  return createSatelliteCacheEntry(entity);
}
...
export function createAgroLavouraPresentation(options={}){
  ...
  const screen=Object.freeze({
    ...
    actions:Object.freeze({
      ...
      removeCachedScene:({id,expectedVersion}={})=>satelliteCache.remove(id,{expectedVersion})
    }),
    ...
    async load(){
      const[fieldRecords,geometryRecords,cacheRecords]=await Promise.all([base.services.repos.fields.list(),base.services.repos.fieldGeometries.list(),satelliteCache.list()]);
      return Object.freeze({fields:Object.freeze(rows(fieldRecords)),geometries:Object.freeze(rows(geometryRecords)),cached:Object.freeze(rows(cacheRecords)),providers:SATELLITE_PROVIDERS,modes:Object.freeze(['vector','imagery','ndvi']),onlineOptional:true,credentialsStored:false});
    }
  });
