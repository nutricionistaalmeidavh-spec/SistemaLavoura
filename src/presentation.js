...
export function createAgroLavouraPresentation({ persistence, localRuntime = null, recovery = null, capabilities = [] } = {}) {
  ...
  async function commercialReadModel(){const [fieldRecords,seasonRecords,operationRecords,harvestRecords,entryRecords,scoutingRecords]=await Promise.all([repos.fields.list(),repos.seasons.list(),repos.operations.list(),repos.harvestLots.list(),finance.list(),repos.scouting.list()]);const fields=rows(fieldRecords),seasons=rows(seasonRecords),operations=rows(operationRecords),harvestLots=rows(harvestRecords),entries=rows(entryRecords),scouting=rows(scoutingRecords);return {fields,seasons,operations,harvestLots,entries,scouting,budget:budgetVsActual({seasons,entries}),results:seasonResults({seasons,harvestLots,entries}),management:managementSnapshot({fields,seasons,operations,harvestLots,entries,scouting})};}
  const screens = {
    ...
    fields:{kind:'table-form',async load(){return{rows:await repos.fields.list(),files:await files.list({entityType:'field'}),references:await referenceData()};},actions:{save:saveField,remove:({id,expectedVersion})=>repos.fields.remove(id,{expectedVersion}),uploadFile:input=>files.upload({...input,entityType:'field'}),removeFile:({id})=>files.remove(id),saveGeometry:input=>repos.fieldGeometries.save(createFieldGeometry(input),{expectedVersion:input.expectedVersion??0}),addScouting:input=>repos.scouting.save(createScoutingRecord(input),{expectedVersion:0})}},
    ...
    harvest:{kind:'table-form',async load(){return{rows:await repos.harvestLots.list(),references:await referenceData()};},actions:{create:input=>repos.harvestLots.save(createHarvestRecord({...input,id:idFor('harvest',input.id)}),{expectedVersion:0})}},
