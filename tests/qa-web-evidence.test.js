...
test('qa web isolates P6 and P7 in fresh Playwright processes',()=>{const partition=partitionE2eFiles(['a.spec.mjs','p6-gis-import.spec.mjs','p7-satellite.spec.mjs','notes.txt']);assert.deepEqual(partition.baseline,['tests/e2e/a.spec.mjs']);assert.deepEqual(partition.p6,['tests/e2e/p6-gis-import.spec.mjs']);assert.deepEqual(partition.p7,['tests/e2e/p7-satellite.spec.mjs']);});
test('qa web fails closed when a specialized regression spec disappears',()=>{assert.throws(()=>partitionE2eFiles(['p6-gis-import.spec.mjs']),/p7-satellite/);});
