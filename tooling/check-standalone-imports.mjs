...
for(const file of files){const source=await readFile(file,'utf8');for(const token of forbidden)if(source.includes(token))failures.push(`${relative(root,file)}: referência externa proibida -> ${token}`);for(const match of source.matchAll(importPattern)){const specifier=match[2];if(!specifier.startsWith('.'))continue;const target=resolve(dirname(file),specifier);const rel=relative(root,target);if(rel.startsWith('..'))failures.push(`${relative(root,file)}: import escapa do repo -> ${specifier}`);}}
