...
let root,host;
const summary={phase:5,productId:contract.productId,status:'failed',screens:[],checks:{}};
try{
  ...
  summary.checks.authentication=true;line('PASS','Autenticação');
  ...
  for(let i=0;i<contract.screens.length;i+=1){
    ...
    summary.screens.push({id,actions:expected.length,loaded:true});line('PASS',`Tela ${i+1}/${contract.screens.length}: ${id}`,`${expected.length} ações`);
  }
  summary.checks.surface=true;
  const security=await import('../src/security.js');
  assert.ok((security.SECURITY_POLICY?.admin??[]).includes('*'));assert.ok(Array.isArray(security.SECURITY_POLICY?.viewer));
  summary.checks.rbacContract=true;line('PASS','Contrato RBAC');
  ...
  summary.checks.backupRestore=true;line('PASS','Backup/restore');
  await host.close();host=null;host=await createStandaloneHost({dataDir:root});
  const again=await host.backend.login({username:'qa-admin',password:credential});await host.backend.validate({sessionId:again.session.id,token:again.token});
  assert.equal((await host.persistence.getRecord('qa.phase5','sentinel')).payload.value,'before');assert.equal((await host.persistence.health()).ok,true);
  summary.checks.restartPersistence=true;summary.status='passed';summary.screenCount=contract.screens.length;summary.actionCount=Object.values(contract.actions).reduce((n,a)=>n+a.length,0);summary.finishedAt=new Date().toISOString();
  await writeFile(summaryPath,JSON.stringify(summary,null,2)+'\n');line('PASS','FASE 5',`${summary.screenCount} telas, ${summary.actionCount} ações`);
}catch(error){summary.error=error.stack??error.message;summary.finishedAt=new Date().toISOString();await mkdir(artifactDir,{recursive:true}).catch(()=>{});await writeFile(summaryPath,JSON.stringify(summary,null,2)+'\n').catch(()=>{});line('FAIL','FASE 5',error.message);process.exitCode=1;}finally{await host?.close?.().catch(()=>{});if(root&&process.env.ARTISYS_QA_KEEP!=='1')await rm(root,{recursive:true,force:true}).catch(()=>{});}
