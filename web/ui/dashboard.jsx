...
export function LavouraDashboard({data,onNavigate,onRun}){
  ...
  async function captureFile(event){const file=event.target.files?.[0];if(!file)return;setBusyAction('capture');setCaptureMessage('');try{const id=globalThis.crypto?.randomUUID?.()??`capture-${Date.now()}`;const result=await onRun('capture',{id,source:'file',name:file.name,mimeType:file.type||'application/octet-stream',bytesBase64:await fileToBase64(file)});setCaptureMessage(result?.ignored?'Captura desativada nas configurações.':`Arquivo ${file.name} capturado na base local.`);}finally{event.target.value='';setBusyAction('');}}
