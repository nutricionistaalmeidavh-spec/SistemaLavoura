...
export function LavouraFieldMode({data,onRun,reload}){
  ...
  async function addScouting(){
    ...
    setBusy(true);try{await onRun('addScouting',{fieldId:selected.id,seasonId:data?.map?.fields?.find(item=>String(item.fieldId)===String(selected.id))?.season?.id??'field-mode',kind:'observação',name:title.trim()||'Ocorrência de campo',severity:1,status:'open',latitude:gps.latitude,longitude:gps.longitude,notes:notes.trim()||null,observedAt:new Date().toISOString()});setTitle('');setNotes('');await reload?.();}finally{setBusy(false);}
  }
