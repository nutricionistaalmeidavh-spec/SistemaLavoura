...
const columns=[
  ...
  {key:'farmUnitId',label:'Unidade/Fazenda'}
];
...
export function LavouraFieldsWorkspace({data,onRun}){
  ...
  async function save(values){setBusy(true);try{await onRun('save',values);setFormOpen(false);setSelected(null);}finally{setBusy(false);}}
  ...
  return <section className="product-workspace" data-testid="fields-workspace">
    <PageHeader eyebrow="Produção" title="Talhões" description="Organize áreas produtivas, unidades e documentos vinculados." actions={<><button type="button" onClick={()=>{setSelected(null);setFormOpen(true);}}>Novo talhão</button>{selectedRow?<button type="button" onClick={()=>setFormOpen(true)}>Editar</button>:null}{selectedRow?<button type="button" onClick={()=>setFileOpen(true)}>Anexar arquivo</button>:null}{selectedRow?<button type="button" className="danger-ghost" onClick={()=>setDeleteOpen(true)}>Excluir</button>:null}</>}/>
    <div className="workspace-kpis"><article className="workspace-kpi"><span>Talhões</span><strong>{rows.length}</strong></article><article className="workspace-kpi"><span>Área cadastrada</span><strong>{totalArea.toLocaleString('pt-BR',{maximumFractionDigits:2})} ha</strong></article><article className="workspace-kpi"><span>Arquivos</span><strong>{files.length}</strong></article></div>
    ...
    <ConfirmDialog open={deleteOpen} title="Excluir talhão" description={selectedRow?`O talhão ${selectedRow.name} será removido.`:''} confirmLabel="Excluir talhão" onCancel={()=>setDeleteOpen(false)} onConfirm={remove}/>
  </section>;
}
