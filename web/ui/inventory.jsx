...
export function LavouraInventoryWorkspace({data,onRun}){
  ...
  return <section className="product-workspace" data-testid="inventory-workspace"><PageHeader eyebrow="Gestão" title="Estoque" description="Controle entradas, saídas e disponibilidade dos insumos." actions={<><button type="button" className="primary-button" onClick={()=>setActionName('receive')}>Registrar entrada</button><button type="button" onClick={()=>setActionName('consume')}>Registrar saída</button></>}/><KpiStrip items={[{label:'Itens com saldo',value:rows.length},{label:'Quantidade em estoque',value:total.toLocaleString('pt-BR')},{label:'Estoque baixo',value:lowStock},{label:'Movimentações',value:events.length}]}/><DataTable columns={columns} rows={rows} emptyTitle="Estoque vazio" emptyDescription="Registre uma entrada para começar o controle de insumos."/>{events.length?<section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Rastreabilidade</span><h3>Movimentações recentes</h3></div><span>{events.length} eventos</span></div><div className="movement-list">{events.slice(-8).reverse().map(event=><article key={event.id}><div><strong>{event.sku}</strong><span>{event.kind==='in'?'Entrada':'Saída'} · {event.quantity}</span></div><small>{dateTime(event.occurredAt)}</small></article>)}</div></section>:null}<Modal open={Boolean(action)} title={action?.label??'Movimentação'} description="A movimentação será registrada no histórico local." onClose={()=>setActionName(null)}><StructuredForm fields={action?.fields??[]} busy={busy} submitLabel={action?.label} onSubmit={submit} onCancel={()=>setActionName(null)}/></Modal></section>;
}
