...
export function LavouraOperationsWorkspace({data,onRun}){
  ...
  return <section className="product-workspace" data-testid="operations-workspace">
    ...
    <KpiStrip items={[...kpis,{label:'Chuva acumulada',value:`${Number(data?.commercial?.climate?.totalRainMm??0).toLocaleString('pt-BR')} mm`},{label:'Alertas P1',value:data?.commercial?.alerts?.length??0}]}/><section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Planejamento</span><h3>Calendário agrícola</h3></div><span>{data?.calendar?.length??0} atividades</span></div><div className="checklist-stack">{(data?.calendar??[]).slice(0,10).map((item,index)=><article className="checklist-card" key={item.id??index}><strong>{item.title??item.name??'Atividade planejada'}</strong><small>{item.start?new Date(item.start).toLocaleDateString('pt-BR'):'Sem data'}</small></article>)}</div></section>
