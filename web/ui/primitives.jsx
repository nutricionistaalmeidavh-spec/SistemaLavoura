...
function FieldControl({definition,value,onChange}){
  const common={id:`field-${definition.name}`,name:definition.name,'aria-label':definition.label,required:Boolean(definition.required),value:value??'',onChange:event=>onChange(definition.type==='checkbox'?event.target.checked:event.target.value)};
  ...
  if(definition.type==='multiselect')return <select id={common.id} name={common.name} aria-label={definition.label} multiple value={Array.isArray(value)?value:[]} onChange={event=>onChange(Array.from(event.target.selectedOptions,option=>option.value))}>{(definition.options??[]).map(option=>{const item=typeof option==='object'?option:{label:String(option),value:option};return <option key={String(item.value)} value={item.value}>{item.label}</option>;})}</select>;
  if(definition.type==='checkbox')return <label className="toggle-field"><input id={common.id} name={common.name} aria-label={definition.label} type="checkbox" checked={Boolean(value)} onChange={event=>onChange(event.target.checked)}/><span>Ativo</span></label>;
  if(definition.type==='file')return <input id={common.id} name={common.name} aria-label={definition.label} type="file" onChange={event=>onChange(event.target.files?.[0]??null)}/>;
