...
import {buildReportSummaryRequest,formatReportSummaryValue} from './report-summary.js';
...
export function LavouraReportsWorkspace({data,onRun}){
  ...
  async function runDialog(values){setBusy(true);try{if(dialog==='summary'){const request=buildReportSummaryRequest(issuedRows,values,definitions);const result=await onRun('summary',request);setLastResult({kind:'summary',type:selectedType,result,summaryValues:values});}else if(dialog==='export'){const rows=issuedRows.map(item=>item.row);const result=await onRun('export',{rows,...values});downloadExportResult(result,{format:values.format,nameBase:'historico-relatorios'});setLastResult({kind:'export',type:selectedType,result});}setDialog(null);}finally{setBusy(false);}}
  ...
  return <section className="product-workspace" data-testid="reports-workspace">
    ...
    <section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Relatório gerencial consolidado</span><h3>Resultado agrícola consolidado</h3></div><span>{report.generatedAt?`Atualizado ${dateTime(report.generatedAt)}`:'Base local'}</span></div><KpiStrip items={[{label:'Área consolidada',value:`${Number(reportIndicators.managedAreaHa??0).toLocaleString('pt-BR',{maximumFractionDigits:2})} ha`},{label:'Receita consolidada',value:money(reportIndicators.incomeMinor)},{label:'Custo consolidado',value:money(reportIndicators.expenseMinor)},{label:'Margem consolidada',value:money(reportIndicators.marginMinor)},{label:'Custo consolidado/ha',value:reportIndicators.costPerHaMinor==null?'—':money(reportIndicators.costPerHaMinor)}]}/><div className="settings-grid"><div><div className="panel-heading"><div><span className="eyebrow">Dentro do relatório</span><h3>Safras consolidadas</h3></div><span>{reportSeasonComparison.length}</span></div><DataTable columns={seasonColumns} rows={reportSeasonComparison} emptyTitle="Sem safras no relatório gerencial"/></div><div><div className="panel-heading"><div><span className="eyebrow">Dentro do relatório</span><h3>Talhões consolidados</h3></div><span>{reportFieldComparison.length}</span></div><DataTable columns={fieldColumns} rows={reportFieldComparison} emptyTitle="Sem talhões no relatório gerencial"/></div></div></section>
