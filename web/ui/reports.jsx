...
export function LavouraReportsWorkspace({data,onRun}){
  ...
  const reportSeasonComparison=Array.isArray(report.seasonComparison)?report.seasonComparison:[];
  const reportFieldComparison=Array.isArray(report.fieldComparison)?report.fieldComparison:[];
  ...
  return <section className="product-workspace" data-testid="reports-workspace">
    <PageHeader eyebrow="Gestão" title="Relatórios" description="Compare safras e talhões, acompanhe indicadores gerenciais e gere documentos persistidos." actions={<><button type="button" onClick={()=>setDialog('summary')}>Resumo das emissões</button><button type="button" onClick={()=>setDialog('export')}>Exportar e baixar</button></>}/>
    ...
    <div className="settings-grid"><section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Comparativo</span><h3>Safra × safra</h3></div><span>{seasonComparison.length} safras</span></div><DataTable columns={seasonColumns} rows={seasonComparison} emptyTitle="Sem safras para comparar"/></section><section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Comparativo</span><h3>Talhão × talhão</h3></div><span>{fieldComparison.length} talhões</span></div><DataTable columns={fieldColumns} rows={fieldComparison} emptyTitle="Sem talhões para comparar"/></section></div>
    <section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Relatório gerencial consolidado</span><h3>Resultado agrícola consolidado</h3></div><span>{report.generatedAt?`Atualizado ${dateTime(report.generatedAt)}`:'Base local'}</span></div><KpiStrip items={[{label:'Área consolidada',value:`${Number(reportIndicators.managedAreaHa??0).toLocaleString('pt-BR',{maximumFractionDigits:2})} ha`},{label:'Receita consolidada',value:money(reportIndicators.incomeMinor)},{label:'Custo consolidado',value:money(reportIndicators.expenseMinor)},{label:'Margem consolidada',value:money(reportIndicators.marginMinor)},{label:'Custo consolidado/ha',value:reportIndicators.costPerHaMinor==null?'—':money(reportIndicators.costPerHaMinor)}]}/><div className="settings-grid"><div><DataTable columns={seasonColumns} rows={reportSeasonComparison} emptyTitle="Sem safras no relatório gerencial"/></div><div><DataTable columns={fieldColumns} rows={reportFieldComparison} emptyTitle="Sem talhões no relatório gerencial"/></div></div></section>
    <section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Alertas inteligentes</span><h3>Atenções do consolidado</h3></div><span>{alerts.length}</span></div>{alerts.length?<div className="checklist-stack">{alerts.map((alert,index)=><article className="checklist-card" key={`${alert.kind??'alert'}-${index}`}><div><strong>{alert.message??'Atenção gerencial'}</strong><span>{alert.severity??'info'}</span></div></article>)}</div>:<p className="muted">Nenhum alerta inteligente no consolidado atual.</p>}</section>
    ...
    <section className="workspace-panel report-actions-panel"><div className="panel-heading"><div><span className="eyebrow">Documento selecionado</span><h3>{definitions[selectedType]?.title??selectedType}</h3></div></div><div className="context-actions"><button type="button" className="primary-button" disabled={busy} onClick={()=>generateCsv()}>Gerar e baixar CSV</button><button type="button" disabled={busy} onClick={generatePdf}>Gerar e baixar PDF</button>{lastResult&&['csv','pdf'].includes(lastResult.kind)?<button type="button" disabled={busy} onClick={issue}>Emitir documento</button>:null}</div></section>
    <section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Histórico</span><h3>Documentos emitidos</h3></div><span>{issuedRows.length}</span></div><DataTable columns={issuedColumns} rows={issuedRows} emptyTitle="Nenhum documento emitido"/></section>
    <Modal open={Boolean(dialog)} title={dialog==='summary'?'Resumir emissões':'Exportar histórico'} onClose={()=>setDialog(null)}><StructuredForm fields={dialog==='summary'?contract.actions.summary.fields:contract.actions.export.fields} busy={busy} submitLabel={dialog==='export'?'Exportar e baixar':undefined} onSubmit={runDialog} onCancel={()=>setDialog(null)}/></Modal>
  </section>;
}
