...
const FEATURE_FLAG_LABELS=Object.freeze({
  'capture.enabled':'Captura de arquivos',
  'files.enabled':'Anexos e arquivos',
  'pdf.enabled':'Relatórios em PDF',
  'checklists.enabled':'Checklists operacionais',
  'catalog.enabled':'Catálogo agrícola',
  'checklists.enforceBeforeOperationComplete':'Exigir checklist antes de concluir operação'
});
const featureFlagLabel=key=>FEATURE_FLAG_LABELS[key]??String(key??'').replaceAll('.',' · ');
...
export function LavouraSettingsWorkspace({data,onRun,screen}){
  ...
  return <section className="product-workspace settings-workspace" data-testid="settings-workspace"><PageHeader eyebrow="Sistema" title="Configurações" description="Preferências locais, segurança operacional, backup e dados auxiliares." actions={<><button type="button" disabled={busy} onClick={backup}>Criar backup</button>{hasRestoreByCode?<button type="button" onClick={()=>setDialog('restore-code')}>Restaurar por código</button>:null}</>}/>
    ...
    <section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Recursos</span><h3>Recursos opcionais</h3></div><span>{flags.length}</span></div><div className="flag-list">{flags.map(([key,value])=><article key={key}><div><strong>{featureFlagLabel(key)}</strong><small>{value?'Ativo':'Desativado'}</small></div><button type="button" className={value?'toggle-button active':'toggle-button'} aria-pressed={Boolean(value)} onClick={()=>toggleFlag(key,value)}>{value?'Ligado':'Desligado'}</button></article>)}</div></section></div>
    ...
    <section className="workspace-panel" data-testid="legal-notice"><div className="panel-heading"><div><span className="eyebrow">Sobre</span><h3>ArtiSys Lavoura 1.0.0</h3></div><span>AGPL-3.0-only</span></div><p className="muted">Software livre distribuído sob GNU Affero General Public License v3.0 only, sem garantia, nos limites permitidos pela lei aplicável. A cópia integral da licença acompanha a instalação no arquivo LICENSE.</p><div className="context-actions"><a href="https://github.com/nutricionistaalmeidavh-spec/SistemaLavoura" target="_blank" rel="noreferrer">Ver código-fonte e licença</a></div></section>
