...
export function LavouraSettingsWorkspace({data,onRun}){
  ...
  async function makeRecoveryCode(){if(!selectedBackup)return;setBusy(true);try{const result=await onRun('recoveryCode',{id:selectedBackup.row.id});setRecoveryCode(result?.code??result);setDialog('recovery-result');}finally{setBusy(false);}}
  async function restoreByCode(values){setBusy(true);try{await onRun('restoreByCode',{code:values.code});setDialog(null);}finally{setBusy(false);}}
  ...
  return <section className="product-workspace settings-workspace" data-testid="settings-workspace"><PageHeader eyebrow="Sistema" title="Configurações" description="Preferências locais, segurança operacional, backup e dados auxiliares." actions={<><button type="button" disabled={busy} onClick={backup}>Criar backup</button><button type="button" onClick={()=>setDialog('restore-code')}>Restaurar por código</button></>}/>
    ...
    <div className="settings-grid"><section className="workspace-panel"><div className="panel-heading"><div><span className="eyebrow">Recuperação</span><h3>Backups locais</h3></div><span>{backups.length}</span></div><DataTable columns={backupColumns} rows={backups} selectedId={selectedBackup?.row?.id??null} onSelect={item=>setSelectedBackup(current=>current?.row?.id===item.row.id?null:item)} emptyTitle="Nenhum backup criado"/>{selectedBackup?<div className="context-actions"><button type="button" onClick={makeRecoveryCode}>Gerar código</button><button type="button" className="danger-ghost" onClick={()=>setConfirmRestore(true)}>Restaurar este backup</button></div>:null}</section>
    ...
    <Modal open={dialog==='restore-code'} title="Restaurar por código" description="Use um código de recuperação gerado anteriormente." onClose={()=>setDialog(null)}><StructuredForm fields={contract.actions.restoreByCode.fields} busy={busy} submitLabel="Restaurar" onSubmit={restoreByCode} onCancel={()=>setDialog(null)}/></Modal>
    <Modal open={dialog==='recovery-result'} title="Código de recuperação" onClose={()=>setDialog(null)}><div className="recovery-code"><strong>{String(recoveryCode??'—')}</strong><p>Guarde este código em local seguro.</p></div></Modal>
